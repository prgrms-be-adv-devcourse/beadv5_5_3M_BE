...
```
✅ values 파일 작성 완료 (전 서비스)
✅ 앱 서비스 9개 배포 완료 (Running)
⏳ CI/CD 교체 전 (현재 수동 helm upgrade)
```
...
```yaml
# PostgreSQL k3s Pod 이전 완료 → ClusterIP DNS 사용
...
env:
  # Service 실제 이름은 chart fullname 헬퍼가 만든 "{release}-microservice" 형태
