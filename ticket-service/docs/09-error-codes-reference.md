...
| `ALREADY_RESERVED` | 409 | 이미 예매된 티켓입니다: {id} | `reserved()` 호출 시 이미 RESERVED |
| `ALREADY_CONFIRMED` | 409 | 이미 확정된 티켓입니다: {id} | `reserved()` / `cancel()` 시 이미 CONFIRMED |
| `ALREADY_HOLD` | 409 | 이미 판매 중지된 티켓입니다: {id} | `reserved()` / `cancel()` 시 HOLD 상태 |
| `ALREADY_PAID` | 409 | 이미 결제된 티켓입니다: {id} | 중복 결제 시도 |
...
| `FULL` | 409 | 티켓이 모두 매진된 스케줄입니다: {id} | AVAILABLE 티켓 없음 (레거시) |
| `EXPIRED` | 409 | 예매가 마감된 스케줄입니다: {id} | endTime - 10분 이후 예매 시도 |
| `ALREADY_CONFIRMED` | 409 | 이미 등록된 스케줄입니다: {id} | Kafka 중복 소비 방지 |
...
| 코드 | HTTP | 메시지 | 발생 상황 |
|------|------|--------|----------|
| `QUEUE_NOT_OPEN` | 409 | 대기열이 열려 있지 않은 스케줄입니다: {id} | `enter()` 시 schedule.status != TICKETING |
| `ALREADY_IN_QUEUE` | 409 | 이미 대기열에 있는 유저입니다: {id} | ZADD 전 ZRANK 조회 결과 존재 |
| `SOLD_OUT` | 409 | 매진된 스케줄입니다: {id} | stock=0 AND RESERVED=0 |
...
### `POST /api/tickets` (레거시 예매)

```
NOT_FOUND (Schedule) → 스케줄 없음
EXPIRED              → 예매 마감
FULL                 → 잔여 좌석 없음
```
